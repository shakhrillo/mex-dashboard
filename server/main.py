...
def search_machines(
    ...
    return crud.search_machines(db=db, fromDate=fromDate, toDate=toDate, toArticle=toArticle, barcodeProductionNo=barcodeProductionNo, ma=ma, status=status)
...
def get_current(machine_id: str, db: Session = Depends(get_db)):
    last_data = db.query(models.MachineData).filter(models.MachineData.machineQrCode == machine_id) # order by date
    # order by date
